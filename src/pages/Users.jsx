	...
	const handleSearch = (e) => {
		if (e.target.value.length === 0) {
			...
			setSearchUser(users?.filter((data) => {
				return data?.name.toLowerCase().includes(e.target.value.toLowerCase()) || data?.email.toLowerCase().includes(e.target.value.toLowerCase()) || data?.plan_start_date.toLowerCase().includes(e.target.value.toLowerCase())
			}))
		}
	}
								...
								<tbody>
									{
										searchUser && searchUser.map((data) => {
											return <tr key={data?.id} className="bg-gray-200 border-b dark:border-gray-700">
												...
												<td colSpan={1} onClick={() => setShowModal(true)} className="px-4 py-3 text-blue-500  cursor-pointer"><BsEye /></td>
											</tr>
										})
